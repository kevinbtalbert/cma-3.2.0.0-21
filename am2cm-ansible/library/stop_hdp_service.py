...
def run_module():
    ...
    is_service_stopped = cluster.is_service_stopped(service_name)

    # if the user is working with this module in only check mode we do not
    # want to make any changes to the environment, just return the current
    # state with no modifications
    if module.check_mode:
        result['changed'] = not is_service_stopped
        module.exit_json(**result)

    if not is_service_stopped:
        is_stopped_successfully = cluster.stop_service(service_name=service_name)
        result['changed'] = is_stopped_successfully
        if not is_stopped_successfully:
            result['message'] = f"Failed to stop {service_name} HDP service!"
            module.fail_json(**result, msg=f"Failed to stop {service_name} HDP service!")
        else:
            result['message'] = f"The {service_name} HDP service has been stopped successfully!"
    else:
        result['changed'] = False
        result['message'] = f"The {service_name} HDP service has already been stopped!"
