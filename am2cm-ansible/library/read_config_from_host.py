...
def run_module():
    ...
    if module.check_mode:
        result['changed'] = True
        result['message'] = f"Configuration {module.params['configuration_name']} from  host" \
                            f" {module.params['host_name']} will be read"
        module.exit_json(**result)

    result['changed'], result['message'] = cluster.read_configuration_from_host(module.params['host_name'],
                                                                                module.params['configuration_name'])
    if result['changed']:
        module.exit_json(**result)
    else:
        module.fail_json(msg="Couldn't get configuration name from host", **result)
