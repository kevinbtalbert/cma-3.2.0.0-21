...
def run_module():
    ...
    module_args = dict(
        ...
        cluster_scanner_configuration=dict(type='dict', required=True)
    )
    ...
    script_path = module.params['cluster_scanner_configuration']['script_path']
    cluster_topology_file = module.params['cluster_scanner_configuration']['cluster_topology_file']
    cm_base_url = f'{cm_protocol}://{cm_hostname}:{cm_port}'

    args = [f'{script_path}/cluster-scanner.sh',
            '--cm-server', cm_base_url,
            '--auth', 'basic',
            '--username', cm_username,
            '--password', cm_password,
            '--cluster-name', cm_cluster_name,
    ...
    if rc != 0:
        ...
        module.fail_json(msg='Exception when calling cluster-scanner.sh', **result)
