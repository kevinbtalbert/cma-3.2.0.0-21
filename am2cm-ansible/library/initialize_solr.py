...
def run_module():
    ...
    if module.check_mode:
        result['changed'] = True
        result['message'] = "Solr will be initialized"
        module.exit_json(**result)

    result['changed'], result['message'] = cluster.run_service_command("solr", "SolrInit")

    if result['changed']:
        module.exit_json(**result)
    else:
        module.fail_json(msg="Exception when attempting to initialize Solr", **result)
