...
def run_module():
    module_args = dict(
        ...
        role_name_filter=dict(type='str', required=True)
    )
    ...
    argument_list = [module.params['service_name'], module.params['role_name_filter']]

    try:
        result['message'] = cluster.get_role_commands(*argument_list)
        module.exit_json(**result)
    except ApiException as e:
        module.fail_json(msg="Exception when looking for role commands list: %s" % e.reason, **result)
