...
def run_module():
    module_args = dict(
        ...
        role_group_filter=dict(type='str', required=True)
    )
    ...
    result['changed'], result['message'] = cluster.get_hostnames_belonging_to_role_group(module
                                                                                         .params['role_group_filter'])

    if result['changed']:
        module.exit_json(**result)
    else:
        module.fail_json(msg="Exception when trying to get hostnames belonging to role group", **result)
