...
def run_module():
    ...
    result['changed'] = cluster.is_service_exists(service_name=service_name)

    if module.check_mode:
        result['message'] = f"Service '{service_name}' will {'' if result['changed'] else 'not'} " \
                            f"be stopped"
        module.exit_json(**result)

    # if service doesn't exist
    if not result['changed']:
        result['message'] = f"Service {service_name} does not exist, so it cannot be stopped"
        module.fail_json(msg=f"Service {service_name} does not exist", **result)

    # if it's not check mode and service exists, stop it
    result['changed'], result['message'] = cluster.stop_service(service_name, module.params['timeout_seconds'])
    if result['changed']:
        module.exit_json(**result)
    else:
        module.fail_json(msg="Exception when attempting to stop service", **result)
