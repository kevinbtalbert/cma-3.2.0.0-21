...
def run_module():
    ...
    if module.check_mode:
        result['changed'] = is_component_started
        module.exit_json(**result)

    if is_component_started:
        wait_for_complete = module.params['wait_for_complete']
        stop_result = cluster.stop_service_component(service_name=service_name,
                                                     component_name=component_name,
                                                     wait_for_complete=wait_for_complete)
        result['changed'] = stop_result['success']
        if not stop_result['success']:
            error_message = f"Failed to stop {component_name} in {service_name} HDP service!"
            result['message'] = error_message
            module.fail_json(**result, msg=error_message)
        else:
            if not wait_for_complete:
                result['tracking_url'] = stop_result['tracking_url']
            result['message'] = f"The {component_name} in {service_name} HDP service has been stopped successfully!"
    else:
        result['changed'] = False
        result['message'] = f"The {component_name} in {service_name} HDP service has already been stopped!"
