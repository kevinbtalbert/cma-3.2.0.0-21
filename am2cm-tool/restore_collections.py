...
BACKUP_SOLR_COLLECTION_URL = '{0}/admin/collections?action=BACKUP&name={1}&collection={2}&location={3}'
RESTORE_SOLR_COLLECTION_URL = '{0}/admin/collections?action=RESTORE&collection={1}&name={2}&location={3}'
...
GET_SYSTEM_INFO_URL = '{0}/admin/info/system?wt=json'
...
DEFAULT_SOLR_VERSION = '8.4.1'
DEFAULT_MAX_READ_BLOCK_SIZE = 100000
DEFAULT_MAX_WRITE_BLOCK_SIZE = 1000000
...
def get_keytab_and_principal(config):
  ...
  if config.has_section('infra_solr'):
    if options.user:
      user=options.user
    elif config.has_option('infra_solr', 'user'):
      user=config.get('infra_solr', 'user')
    if kerberos_enabled == 'true':
      if options.keytab:
        keytab=options.keytab
      elif config.has_option('infra_solr', 'keytab'):
        keytab=config.get('infra_solr', 'keytab')
      if options.principal:
        principal=options.principal
      elif config.has_option('infra_solr', 'principal'):
        principal=config.get('infra_solr', 'principal')
  return keytab, principal

def get_hdfs_keytab_and_principal(config):
  kerberos_enabled = 'false'
  keytab=None
  principal=None
  if config.has_section('cluster') and config.has_option('cluster', 'kerberos_enabled'):
  ...
  if config.has_section('hdfs'):
    if config.has_option('hdfs', 'user'):
      user=config.get('hdfs', 'user')
    if kerberos_enabled == 'true':
      if config.has_option('hdfs', 'keytab'):
        keytab=config.get('hdfs', 'keytab')
      if config.has_option('hdfs', 'principal'):
        principal=config.get('hdfs', 'principal')
  return keytab, principal
...
def create_infra_solr_client_command(options, config, command, appendZnode=False):
  ...
  if config.has_section('infra_solr'):
    ...
    elif options.zk_connect_string:
      zkConnectString=options.zk_connect_string
    elif config.has_option('infra_solr', 'zk_connect_string'):
      zkConnectString=config.get('infra_solr', 'zk_connect_string')
  if kerberos_enabled == 'true':
    zk_principal_user = options.zk_principal_user if options.zk_principal_user else config.get('infra_solr', 'zk_principal_user') if config.has_option('infra_solr', 'zk_principal_user') else 'zookeeper'
  ...
  if appendZnode:
    if options.znode:
      znode_to_append=options.znode
    elif config.has_option('infra_solr', 'znode'):
...
def dump(options, accessor, parser, config, collection, index_location):
  """
  Send dump solr collection custom command request to ambari-server
  """
  cluster = config.get('ambari_server', 'cluster')

  # component_hosts = get_solr_hosts(options, accessor, cluster)
  parameters = fill_parameters(options, config, collection, index_location)
  parameters['solr_max_read_block_size'] = options.max_read_block_size
  parameters['solr_max_write_block_size'] = options.max_write_block_size

  local_host = socket.getfqdn()
  hosts = [local_host]

  cmd_request = create_command_request("DUMP", parameters, hosts, cluster, 'Dump Solr Collection: ' + collection)
  return post_json(accessor, CLUSTERS_URL.format(cluster) + REQUESTS_API_URL, cmd_request)


def reindex(options, accessor, parser, config, collection, index_location):
  """
  Send reindex solr collection custom command request to ambari-server
  """
  cluster = config.get('ambari_server', 'cluster')
  ...
  cmd_request = create_command_request("REINDEX", parameters, hosts, cluster, 'Reindex Solr Collection: ' + collection)
  return post_json(accessor, CLUSTERS_URL.format(cluster) + REQUESTS_API_URL, cmd_request)
...
def get_solr_urls(options, config, collection=None, collections_json=None):
  solr_urls = []
  solr_hosts = None
  solr_port = "8886"
  solr_protocol = "http"
  if options.port:
    solr_port = options.port
  elif config.has_section("infra_solr") and config.has_option("infra_solr", "port"):
    solr_port = config.get('infra_solr', 'port')
  if options.protocol:
    solr_protocol = options.protocol
  elif config.has_section("infra_solr") and config.has_option("infra_solr", "protocol"):
    solr_protocol = config.get('infra_solr', 'protocol')
  if options.hosts:
    solr_hosts=options.hosts
  elif config.has_section('infra_solr') and config.has_option('infra_solr', 'hosts'):
    solr_hosts = config.get('infra_solr', 'hosts')

  splitted_solr_hosts = solr_hosts.split(',')
  if collection and collections_json:
...
def create_check_path_exists_command(config, user, path):
  kerberos_enabled='false'
  if config.has_section('cluster') and config.has_option('cluster', 'kerberos_enabled'):
    kerberos_enabled=config.get('cluster', 'kerberos_enabled')
  keytab, principal=get_hdfs_keytab_and_principal(config)
  #command_user="sudo -u {0}".format(user)
  command_user="su -l {0} -s /bin/bash -c ".format(user)
  copy_cmd = '{0} "kinit -kt {1} {2};hdfs dfs -ls {3}"'.format(command_user, keytab, principal, path) \
    if kerberos_enabled == 'true' else '{0} "hdfs dfs -ls {1}"  '.format(command_user, path)
  logger.debug("HDFS command: {0}".format(copy_cmd))
  return copy_cmd

def check_path_exists_in_hdfs(config, path):
  user='hdfs'
  if config.has_section('hdfs'):
    if config.has_option('hdfs', 'user'):
      user=config.get('hdfs', 'user')
  copy_cmd = create_check_path_exists_command(config, user, path)
  process = Popen(copy_cmd, stdout=PIPE, stderr=PIPE, shell=True)
  out, err = process.communicate()
  if process.returncode != 0 and 'No such file or directory' not in str(err):
    raise Exception("{0} command failed: {1}".format(copy_cmd, str(err)))
  response=str(out)
  if 'Found' in response:
    print 'Path {0} exists in HDFS was {1}SUCCESSFUL{2}'.format(path, colors.OKGREEN, colors.ENDC)
    return 'success'
  elif 'No such file or directory' in str(err):
    return 'failure'
  else:
    raise Exception(("Path {0} does not exist in HDFS {1} failed.({1}) Response: {1}".format(path, str(out))))


def create_hdfs_change_owner_command(config, user, owner_user, path):
  kerberos_enabled='false'
  if config.has_section('cluster') and config.has_option('cluster', 'kerberos_enabled'):
    kerberos_enabled=config.get('cluster', 'kerberos_enabled')
  keytab, principal=get_hdfs_keytab_and_principal(config)
  #command_user="sudo -u {0}".format(user)
  command_user="su -l {0} -s /bin/bash -c ".format(user)
  if path.startswith("/tmp") and user=='root':
    command_user='sudo su - -s /bin/bash -c '
    copy_cmd = '{0} "kinit -kt {1} {2};hdfs dfs -chown -R {3}:{3} {4}"'.format(command_user, keytab, principal, owner_user, path) \
      if kerberos_enabled == 'true' else '{0} "hdfs dfs -chown -R {1}:{1} {2}" '.format(command_user, owner_user, path)
  elif path.startswith("/tmp") and user!='root':
    copy_cmd = '{0} "kinit -kt {1} {2};hdfs dfs -chown -R {3}:{3} {4}"'.format(command_user, keytab, principal, owner_user, path) \
      if kerberos_enabled == 'true' else '{0} "hdfs dfs -chown -R {1}:{1} {2}" '.format(command_user, owner_user, path)
  else:
    copy_cmd = '{0} "kinit -kt {1} {2};hdfs dfs -chown -R {3}:{3} {4}"'.format(command_user, keytab, principal, owner_user, path) \
...
def change_ownership_in_hdfs(config, path):
  user='hdfs'
  if config.has_section('hdfs'):
    if config.has_option('hdfs', 'user'):
      user=config.get('hdfs', 'user')

  owner_user='infra-solr'
  if options.user:
    owner_user=options.user
  elif config.has_section('infra_solr'):
    if config.has_option('infra_solr', 'user'):
      owner_user=config.get('infra_solr', 'user')
  copy_cmd = create_hdfs_change_owner_command(config, user, owner_user, path)
  process = Popen(copy_cmd, stdout=PIPE, stderr=PIPE, shell=True)
  out, err = process.communicate()
  if process.returncode != 0:
    print('Change ownership command failed with {0} user, trying the command directly as root user'.format(user))
    copy_cmd=create_hdfs_change_owner_command(config, 'root', owner_user, path)
    process = Popen(copy_cmd, stdout=PIPE, stderr=PIPE, shell=True)
    out, err = process.communicate()
    if process.returncode != 0:
      raise Exception("{0} command failed: {1}".format(copy_cmd, str(err)))
    response=str(out)
    if len(response) == 0 or 'success' in response:
      print 'Change ownership for path {0} to user {1} in HDFS was {2}SUCCESSFUL{3}'.format(path, owner_user, colors.OKGREEN, colors.ENDC)
      return 'success'
    else:
      raise Exception("Change ownership for path {0} to user {1} in HDFS failed. ({2}) Response: {2}".format(path, owner_user, str(out)))
  response=str(out)
  if len(response) == 0 or 'success' in response:
    print 'Change ownership for path {0} to user {1} in HDFS was {2}SUCCESSFUL{3}'.format(path, owner_user, colors.OKGREEN, colors.ENDC)
    return 'success'
...
def create_hdfs_copy_command(config, source, destination):
  user='infra-solr'
  if options.user:
    user=options.user
  elif config.has_section('infra_solr'):
  ...
  if config.has_section('cluster') and config.has_option('cluster', 'kerberos_enabled'):
    kerberos_enabled=config.get('cluster', 'kerberos_enabled')
  keytab, principal=get_keytab_and_principal(config)
  use_hdfs_user="sudo -u {0}".format(user)
  copy_cmd = '{0} kinit -kt {1} {2} && {3} hdfs dfs  -copyFromLocal -d {4} {5}'.format(use_hdfs_user, keytab, principal, use_hdfs_user, source, destination) \
    if kerberos_enabled == 'true' else '{0} hdfs dfs -copyFromLocal -d {1} {2}'.format(use_hdfs_user, source, destination)
  logger.debug("HDFS command: {0}".format(copy_cmd))
  return copy_cmd

def copy_data_to_hdfs(config, source, destination):
  copy_cmd = create_hdfs_copy_command(config, source, destination)
  process = Popen(copy_cmd, stdout=PIPE, stderr=PIPE, shell=True)
  out, err = process.communicate()
  if process.returncode != 0:
    raise Exception("{0} command failed: {1}".format(copy_cmd, str(err)))
  response=str(out)
  if len(response) == 0 or 'success' in response:
    print 'Copy from {0} to HDFS {1} was {2}SUCCESSFUL{3}'.format(source, destination, colors.OKGREEN, colors.ENDC)
    return 'success'
  else:
    raise Exception("Copy from {0} to HDFS {1} failed. ({1}) Response: {1}".format(source, destination, str(out)))

def check_collection_exists(options, config, collection):
  solr_urls = get_solr_urls(options, config, collection, None)
  request = LIST_SOLR_COLLECTION_URL.format(get_random_solr_url(solr_urls, options))
  logger.debug("Solr request: {0}".format(request))
  check_collection_json_cmd=create_solr_api_request_command(request, config)
  process = Popen(check_collection_json_cmd, stdout=PIPE, stderr=PIPE, shell=True)
  out, err = process.communicate()
  if process.returncode != 0:
    raise Exception("{0} command failed: {1}".format(check_collection_json_cmd, str(err)))
  response=json.loads(str(out))
  if collection in response['collections']:
    print '{0}Collection {1} already exists in Solr.{2}'.format(colors.WARNING, collection, colors.ENDC)
    return 'exists'
  else:
    logger.debug("Collection ('{0}') does not exist in Solr. ({1}) Response: {1}".format(collection, str(out)))
    return 'notexist'


def backup_full_collection(options, config, collection, backup_path):
  solr_urls = get_solr_urls(options, config, collection, None)
  request = BACKUP_SOLR_COLLECTION_URL.format(get_random_solr_url(solr_urls, options), collection, collection, backup_path)
  logger.debug("Solr request: {0}".format(request))
  backup_collection_json_cmd=create_solr_api_request_command(request, config)
  process = Popen(backup_collection_json_cmd, stdout=PIPE, stderr=PIPE, shell=True)
  out, err = process.communicate()
  if process.returncode != 0:
    raise Exception("{0} command failed: {1}".format(backup_collection_json_cmd, str(err)))
  response=json.loads(str(out))
  if 'success' in response:
    print 'Backup for collection {0} was {1}SUCCESSFUL{2}'.format(collection, colors.OKGREEN, colors.ENDC)
    return collection
  else:
    raise Exception("Backup for collection ('{0}') failed. ({1}) Response: {1}".format(collection, str(out)))

def restore_full_collection(options, config, collection, restore_path):
  collection_check=check_collection_exists(options, config, collection)
  path_exists_in_hdfs=check_path_exists_in_hdfs(config, restore_path)
  if collection_check.lower()=='notexist' and path_exists_in_hdfs.lower()=='success':
    solr_urls = get_solr_urls(options, config, collection, None)
    request = RESTORE_SOLR_COLLECTION_URL.format(get_random_solr_url(solr_urls, options), collection, collection, restore_path)
    logger.debug("Solr request: {0}".format(request))
    restore_collection_json_cmd=create_solr_api_request_command(request, config)
    process = Popen(restore_collection_json_cmd, stdout=PIPE, stderr=PIPE, shell=True)
    out, err = process.communicate()
    if process.returncode != 0:
      raise Exception("{0} command failed: {1}".format(restore_collection_json_cmd, str(err)))
    response=json.loads(str(out))
    if 'success' in response:
      print 'Restore for collection {0} was {1}SUCCESSFUL{2}'.format(collection, colors.OKGREEN, colors.ENDC)
      return collection
    else:
      raise Exception("Restore for collection ('{0}') failed. ({1}) Response: {1}".format(collection, str(out)))
  else:
    if collection_check.lower() == 'exists':
      print("{0}Skipping restoring for collection: {1}, as it already exists.{2}".format(colors.WARNING, collection,colors.ENDC))
    if path_exists_in_hdfs.lower()=='failure':
      print("{0}Skipping restoring for collection: {1}, as HDFS backup path {2} does not exist.{3}".format(colors.WARNING, collection, restore_path,colors.ENDC))


def delete_collection(options, config, collection, path):
  path_exists_in_hdfs=check_path_exists_in_hdfs(config, path+'/'+collection)
  if path_exists_in_hdfs.lower()=='success':
    ...
    print("{0}Skipping deletion for collection: {1}, backup for same does not exist in HDFS path {2}.{3}".format(colors.WARNING, collection,path+'/'+collection,colors.ENDC))
...
def get_solr_version(options, config, solr_urls):
  request = GET_SYSTEM_INFO_URL.format(get_random_solr_url(solr_urls, options))
  logger.debug("Solr request: {0}".format(request))
  get_system_info_cmd=create_solr_api_request_command(request, config)
  process = Popen(get_system_info_cmd, stdout=PIPE, stderr=PIPE, shell=True)
  out, err = process.communicate()
  if process.returncode != 0:
    raise Exception("{0} command failed: {1}".format(get_system_info_cmd, str(err)))
  response=json.loads(str(out))
  if 'lucene' in response:
    if 'solr-spec-version' in response['lucene']:
      return response['lucene']['solr-spec-version']
  else:
    raise Exception("Get system info failed. ({1}) Response: {1}".format(str(out)))
...
def upgrade_ranger_schema(options, config, service_filter):
  ...
  if is_ranger_available(config, service_filter):
    if options.znode:
      solr_znode=options.znode
    elif config.has_section('infra_solr') and config.has_option('infra_solr', 'znode'):
...
def do_dump_request(options, accessor, parser, config, collection, index_location):
  sys.stdout.write("Sending dump collection request ('{0}') to Ambari to process (backup destination: '{1}')..."
                   .format(collection, index_location))
  sys.stdout.flush()
  response = dump(options, accessor, parser, config, collection, index_location)
  request_id = get_request_id(response)
  sys.stdout.write(colors.OKGREEN + 'DONE\n' + colors.ENDC)
  sys.stdout.flush()
  print 'Dump command request id: {0}'.format(request_id)
  if options.async:
    print "Dump {0} collection request sent to Ambari server. Check Ambari UI about the results.".format(collection)
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Ambari request with id {0} ...".format(request_id))
    sys.stdout.flush()
    cluster = config.get('ambari_server', 'cluster')
    monitor_request(options, accessor, cluster, request_id, 'Dump Solr collection: ' + collection)
    print "Dumping collection '{0}'... {1}DONE{2}".format(collection, colors.OKGREEN, colors.ENDC)


def do_reindex_request(options, accessor, parser, config, collection, index_location):
  sys.stdout.write("Sending upload collection request ('{0}') to Ambari to process (backup location: '{1}')..."
                   .format(collection, index_location))
  sys.stdout.flush()
  response = reindex(options, accessor, parser, config, collection, index_location)
  request_id = get_request_id(response)
  sys.stdout.write(colors.OKGREEN + 'DONE\n' + colors.ENDC)
  sys.stdout.flush()
  print 'Upload command request id: {0}'.format(request_id)
  if options.async:
    print "Upload {0} collection request sent to Ambari server. Check Ambari UI about the results.".format(collection)
    sys.exit(0)
  else:
    sys.stdout.write("Start monitoring Ambari request with id {0} ...".format(request_id))
    sys.stdout.flush()
    cluster = config.get('ambari_server', 'cluster')
    monitor_request(options, accessor, cluster, request_id, 'Restore Solr collection: ' + collection)
    print "Uploading collection '{0}'... {1}DONE{2}".format(collection, colors.OKGREEN, colors.ENDC)
...
def get_atlas_index_location(collection, config, options):
  ...
  if options.index_location:
  ...
  elif options.atlas_index_location:
    ...
    print "'backup_path'is missing from config file and --index-location or --atlas-index-location options are missing as well." \
      .format(collection, colors.FAIL, colors.ENDC)
    sys.exit(1)
  return atlas_index_location


def get_ranger_hdfs_base_path(options, config):
  hdfs_base_path = None
  if options.ranger_hdfs_base_path:
    ...
    hdfs_base_path = config.get('ranger_collection', 'backup_path')
  return hdfs_base_path


def get_atlas_hdfs_base_path(options, config):
  hdfs_base_path = None
  if options.atlas_hdfs_base_path:
    ...
    hdfs_base_path = config.get('atlas_collections', 'backup_path')
  return hdfs_base_path
...
def is_dump_collection(options, config):
  solr_version = config.get('infra_solr', 'version')
  return int(solr_version[0]) < int(options.index_version[0])-1 if solr_version else True


def dump_collections(options, accessor, parser, config, service_filter):
  collections=list_collections(options, config, COLLECTIONS_DATA_JSON_LOCATION.format("dump_collections.json"), include_number_of_docs=True)
  collections=filter_collections(options, collections)
  num_docs_map = get_number_of_docs_map(COLLECTIONS_DATA_JSON_LOCATION.format("dump_collections.json"))
  if is_ranger_available(config, service_filter):
    ranger_collection = config.get('ranger_collection', 'ranger_collection_name')
    if ranger_collection in collections:
      if is_collection_empty(num_docs_map, ranger_collection):
        print "Collection '{0}' is empty. Dump is not required.".format(ranger_collection)
      else:
        ranger_index_location=get_ranger_index_location(ranger_collection, config, options)
        do_dump_request(options, accessor, parser, config, ranger_collection, ranger_index_location)
    else:
      print 'Collection {0} does not exist or filtered out. Skipping dump operation.'.format(ranger_collection)
  if is_atlas_available(config, service_filter):
    fulltext_index_collection = config.get('atlas_collections', 'fulltext_index_name')
    if fulltext_index_collection in collections:
      if is_collection_empty(num_docs_map, fulltext_index_collection):
        print "Collection '{0}' is empty. Dump is not required.".format(fulltext_index_collection)
      else:
        fulltext_index_location = get_atlas_index_location(fulltext_index_collection, config, options)
        do_dump_request(options, accessor, parser, config, fulltext_index_collection, fulltext_index_location)
    else:
      print 'Collection {0} does not exist or filtered out. Skipping dump operation.'.format(fulltext_index_collection)
    vertex_index_collection = config.get('atlas_collections', 'vertex_index_name')
    if vertex_index_collection in collections:
      if is_collection_empty(num_docs_map, vertex_index_collection):
        print "Collection '{0}' is empty. Dump is not required.".format(vertex_index_collection)
      else:
        vertex_index_location = get_atlas_index_location(vertex_index_collection, config, options)
        do_dump_request(options, accessor, parser, config, vertex_index_collection, vertex_index_location)
    else:
      print 'Collection {0} does not exist or filtered out. Skipping dump operation.'.format(vertex_index_collection)
    edge_index_collection = config.get('atlas_collections', 'edge_index_name')
    if edge_index_collection in collections:
      if is_collection_empty(num_docs_map, edge_index_collection):
        print "Collection '{0}' is empty. Dump is not required.".format(edge_index_collection)
      else:
        edge_index_location = get_atlas_index_location(edge_index_collection, config, options)
        do_dump_request(options, accessor, parser, config, edge_index_collection, edge_index_location)
    else:
      print 'Collection {0} does not exist or filtered out. Skipping dump operation.'.format(edge_index_collection)


def is_upgrading_one_major_version():
  solr_urls = get_solr_urls(options, config)

  target_solr_version = get_solr_version(options, config, solr_urls)
  target_solr_major_version = int(target_solr_version[0])

  source_solr_version = config.get('infra_solr', 'version')
  source_solr_major_version = int(source_solr_version[0]) if source_solr_version else 0

  return target_solr_major_version - source_solr_major_version == 1
...
def restore_collections(options, accessor, parser, config, service_filter):
  ...
  if 'RANGER' in service_filter and config.has_section('ranger_collection') and config.has_option('ranger_collection', 'enabled') \
    ...
    hdfs_base_path = get_ranger_hdfs_base_path(options, config)
  ...
  if is_atlas_available(config, service_filter):
    hdfs_base_path = get_atlas_hdfs_base_path(options, config)
...
def reindex_collections(options, accessor, parser, config, service_filter):
  collections=list_collections(options, config, COLLECTIONS_DATA_JSON_LOCATION.format("reindex_collections.json"))
  collections=filter_collections(options, collections)
  num_docs_map = get_number_of_docs_map(COLLECTIONS_DATA_JSON_LOCATION.format("dump_collections.json"))
  if 'RANGER' in service_filter and config.has_section('ranger_collection') and config.has_option('ranger_collection', 'enabled') \
      and config.get('ranger_collection', 'enabled') == 'true':
    ranger_collection = config.get('ranger_collection', 'ranger_collection_name')
    if ranger_collection in collections and not is_collection_empty(num_docs_map, ranger_collection):
      ranger_index_location=get_ranger_index_location(ranger_collection, config, options)
      do_reindex_request(options, accessor, parser, config, ranger_collection, ranger_index_location)
    else:
      print "Collection '{0}' does not exist or filtered out. Skipping reindex operation.".format(ranger_collection)

  if is_atlas_available(config, service_filter):
    fulltext_index_collection = config.get('atlas_collections', 'fulltext_index_name')
    if fulltext_index_collection in collections and not is_collection_empty(num_docs_map, fulltext_index_collection):
      fulltext_index_location=get_atlas_index_location(fulltext_index_collection, config, options)
      do_reindex_request(options, accessor, parser, config, fulltext_index_collection, fulltext_index_location)
    else:
      print "Collection '{0}' does not exist or filtered out. Skipping reindex operation.".format(fulltext_index_collection)

    edge_index_collection = config.get('atlas_collections', 'edge_index_name')
    if edge_index_collection in collections and not is_collection_empty(num_docs_map, edge_index_collection):
      edge_index_location=get_atlas_index_location(edge_index_collection, config, options)
      do_reindex_request(options, accessor, parser, config, edge_index_collection, edge_index_location)
    else:
      print "Collection '{0}' does not exist or filtered out. Skipping reindex operation.".format(edge_index_collection)

    vertex_index_collection = config.get('atlas_collections', 'vertex_index_name')
    if vertex_index_collection in collections and not is_collection_empty(num_docs_map, vertex_index_collection):
      vertex_index_location=get_atlas_index_location(vertex_index_collection, config, options)
      do_reindex_request(options, accessor, parser, config, vertex_index_collection, vertex_index_location)
    else:
      print "Collection '{0}' does not exist or filtered out. Skipping reindex operation.".format(vertex_index_collection)
...
if __name__=="__main__":
  ...
  parser.add_option("-a", "--action", dest="action", type="string", help="delete-new-solr-collections | change-ownership-in-hdfs | full-restore")
  ...
  parser.add_option("--version", dest="index_version", type="string", default=DEFAULT_SOLR_VERSION, help="lucene index version for migration (default is {0})".format(DEFAULT_SOLR_VERSION))
  ...
  parser.add_option("--max-read-block-size", dest="max_read_block_size", type="int", default=100000, help="Max number of records per request for document dump/upload (default is {0})".format(DEFAULT_MAX_READ_BLOCK_SIZE))
  parser.add_option("--max-write-block-size", dest="max_write_block_size", type="int", default=100000, help="Max number of records in the output files for document dump (default is {0})".format(DEFAULT_MAX_WRITE_BLOCK_SIZE))
  parser.add_option('--protocol', dest='protocol', default=None, type='string', help='CM Solr protocol should be either http or https.')
  parser.add_option('--hosts', dest='hosts', default=None, type='string', help='CM Solr hosts should be passed as comma separated f.q.d.n')
  parser.add_option('--zk-connect-string', dest='zk_connect_string', default=None, type='string', help='CM Solr Zookeeper connection string. (e.g.: localhost:2181)')
  parser.add_option('--znode', dest='znode', default=None, type='string', help='Solr Zookeeper znode configured in CM. (e.g.: /solr-infra)')
  parser.add_option('--zk-principal-user', dest='zk_principal_user', default=None, type='string', help='Principal for Zookeeper Server used by Solr configured in CM. (e.g.: zookeeper)')
  parser.add_option('--user', dest='user', default=None, type='string', help='Solr service user configured in CM. (e.g.: solr)')
  parser.add_option('--port', dest='port', default=None, type='string', help='Solr service port configured in CM. (e.g.: 8983)')
  parser.add_option('--keytab', dest='keytab', default=None, type='string', help='CM Solr service keytab with absolute path. (e.g.: /var/lib/solr/solr.keytab)')
  parser.add_option('--principal', dest='principal', default=None, type='string', help='Solr service principal configured in CM. (e.g.: solr@EXAMPLE.COM)')
  ...
  if options.action is None:
    ...
    if config.has_section('ambari_server'):
      ...
      if solr_hosts and local_host not in solr_hosts.split(","):
        print "{0}WARNING{1}: Host '{2}' is not found in Solr hosts ({3}). Migration commands won't work from here." \
          .format(colors.WARNING, colors.ENDC, local_host, solr_hosts)
      if options.action.lower() == 'full-restore':
        if is_ranger_available(config, service_filter):
          restore_full_collection(options, config, config.get('ranger_collection','ranger_collection_name'), get_ranger_hdfs_base_path(options, config))
        else:
          print '{0}Ranger is not enabled, skipping collection restore.{1}'.format(colors.WARNING, colors.ENDC)
        if is_atlas_available(config, service_filter):
          restore_full_collection(options, config, config.get('atlas_collections','fulltext_index_name'), get_atlas_hdfs_base_path(options, config) + '/' + config.get('atlas_collections','fulltext_index_name'))
          restore_full_collection(options, config, config.get('atlas_collections','edge_index_name'), get_atlas_hdfs_base_path(options, config) + '/' + config.get('atlas_collections','edge_index_name'))
          restore_full_collection(options, config, config.get('atlas_collections','vertex_index_name'), get_atlas_hdfs_base_path(options, config) + '/' + config.get('atlas_collections','vertex_index_name'))
        else:
          print '{0}Atlas is not enabled, skipping collection restore.{1}'.format(colors.WARNING, colors.ENDC)
      elif options.action.lower() == 'copy-to-hdfs':
        if is_ranger_available(config, service_filter) and is_atlas_available(config, service_filter) and get_ranger_hdfs_base_path(options, config) == get_atlas_hdfs_base_path(options, config):
          copy_data_to_hdfs(config, get_ranger_hdfs_base_path(options, config) + "/", config.get('infra_solr','znode'))
        else:
          if is_ranger_available(config, service_filter):
            copy_data_to_hdfs(config, get_ranger_hdfs_base_path(options, config) + "/", config.get('infra_solr','znode'))
          else:
            print '{0}Ranger is not enabled, skipping copy to hdfs.{1}'.format(colors.WARNING, colors.ENDC)
          if is_atlas_available(config, service_filter):
            copy_data_to_hdfs(config, get_atlas_hdfs_base_path(options, config) + "/", config.get('infra_solr','znode'))
          else:
            print '{0}Atlas is not enabled, skipping copy to hdfs.{1}'.format(colors.WARNING, colors.ENDC)
      elif options.action.lower() == "change-ownership-in-hdfs" :
        if is_ranger_available(config, service_filter) and is_atlas_available(config, service_filter) and get_ranger_hdfs_base_path(options, config) == get_atlas_hdfs_base_path(options, config):
          change_ownership_in_hdfs(config, get_ranger_hdfs_base_path(options, config))
        else:
          if is_ranger_available(config, service_filter):
            change_ownership_in_hdfs(config, get_ranger_hdfs_base_path(options, config))
          else:
            print '{0}Ranger is not enabled, skipping change of ownership in HDFS.{1}'.format(colors.WARNING, colors.ENDC)
          if is_atlas_available(config, service_filter):
            change_ownership_in_hdfs(config, get_atlas_hdfs_base_path(options, config))
          else:
            print '{0}Atlas is not enabled, skipping change of ownership in HDFS.{1}'.format(colors.WARNING, colors.ENDC)
      elif options.action.lower() == "delete-new-solr-collections" :
        if is_ranger_available(config, service_filter):
          delete_collection(options, config, config.get('ranger_collection','ranger_collection_name'),  get_ranger_hdfs_base_path(options, config))
        else:
          print '{0}Ranger is not enabled, skipping deletion of existing collection.{1}'.format(colors.WARNING, colors.ENDC)
        if is_atlas_available(config, service_filter):
          delete_collection(options, config, config.get('atlas_collections','fulltext_index_name'), get_atlas_hdfs_base_path(options, config))
          delete_collection(options, config, config.get('atlas_collections','edge_index_name'), get_atlas_hdfs_base_path(options, config))
          delete_collection(options, config, config.get('atlas_collections','vertex_index_name'), get_atlas_hdfs_base_path(options, config))
        else:
          print '{0}Atlas is not enabled, skipping deletion of existing collection.{1}'.format(colors.WARNING, colors.ENDC)
      else:
        parser.print_help()
        print 'action option is invalid (available actions: delete-new-solr-collections | change-ownership-in-hdfs | full-restore)'
        sys.exit(1)
